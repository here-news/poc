...
function LiveSignals() {
  ...
  const fetchStories = async (isBackground = false) => {
    try {
      ...
      const response = await fetch(`/storychat/api/stories?limit=12&min_coherence=${debouncedCoherence}`)
  ...
  const loadMoreStories = async () => {
    ...
    try {
      ...
      const response = await fetch(`/storychat/api/stories?limit=12&offset=${offset}&min_coherence=${debouncedCoherence}`)
      const data = await response.json()
