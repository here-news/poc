...
function StoryChatPage() {
          ...
          for (const storyId in currentMessages) {
            ...
            for (let i = 0; i < messages.length; i++) {
              ...
              if (msg.linkPreview?.taskId && (msg.linkPreview.status === 'processing' || msg.linkPreview.status === 'fetching')) {
                ...
                try {
                  const response = await fetch(`/storychat/api/task/${msg.linkPreview.taskId}`)
                  ...
                  if (data.status === 'completed' || data.status === 'failed') {
                    ...
                    if (storyMatch?.story_id && !storyTitle) {
                      try {
                        const storyResponse = await fetch(`/storychat/api/story/${storyMatch.story_id}`)
  ...
  // Extract base path (e.g., '/storychat' from '/storychat/anything') for portability
  ...
  useEffect(() => {
    const fetchNewsCuration = async () => {
      try {
        ...
        const response = await fetch('/storychat/api/news-curation')
        const data = await response.json()
        if (data.success) {
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        const response = await fetch('/storychat/api/stories?limit=50&min_coherence=0.5')
        const data = await response.json()
        if (data.stories) {
          ...
          if (storyId) {
            if (storyId === GLOBAL_CHAT_ID) {
              ...
              if (story) {
                ...
                const storyResponse = await fetch(`/storychat/api/stories/${storyId}`)
                const storyData = await storyResponse.json()
  ...
  const loadStoryData = async (id: string) => {
    ...
    try {
      const response = await fetch(`/storychat/api/story/${id}/claims`)
      const data = await response.json()
  ...
  const startTaskPolling = (taskId: string, url: string) => {
    const pollInterval = setInterval(async () => {
      try {
        const response = await fetch(`/storychat/api/task/${taskId}`)
        const data = await response.json()
    ...
    try {
      ...
      if (selectedStory.id === GLOBAL_CHAT_ID && detectedURL) {
        ...
        try {
          ...
          const extractResponse = await fetch('/storychat/api/extract', {
            method: 'POST',
      ...
      if (selectedStory.id === GLOBAL_CHAT_ID) {
        ...
        // Story-specific chat
        endpoint = `/storychat/api/story/${selectedStory.id}/chat`
        body = {
          message: trimmed,
          conversation_history: conversationHistory
        }
