...
export interface QuestWebSocketMessage {
  type: 'evidence_submitted' | 'probability_update' | 'comment_added' | 'bounty_added' | 'quest_converged' | 'hypothesis_fork' | 'pong';
  quest_id: string;
  data: any;
  timestamp: string;
}
...
export interface QuestWebSocketHandlers {
  ...
  onHypothesisFork?: (data: any) => void;
}
...
export function useQuestWebSocket(questId: string | undefined, handlers: QuestWebSocketHandlers) {
        ...
        try {
          ...
          switch (message.type) {
            case 'evidence_submitted':
              handlersRef.current.onEvidenceSubmitted?.(message.data);
              break;
            case 'probability_update':
              handlersRef.current.onProbabilityUpdate?.(message.data);
              break;
            case 'comment_added':
              handlersRef.current.onCommentAdded?.(message.data);
              break;
            case 'bounty_added':
              handlersRef.current.onBountyAdded?.(message.data);
              break;
            case 'quest_converged':
              handlersRef.current.onQuestConverged?.(message.data);
              break;
            case 'hypothesis_fork':
              handlersRef.current.onHypothesisFork?.(message.data);
              break;
            case 'pong':
  ...
  }, [questId]); // Only questId dependency - handlers are in ref
